...
🚀 使用方法
方法1: 直接在线执行（推荐）
bashbash <(curl -Ls https://raw.githubusercontent.com/你的用户名/scripts/main/change_ssh_port.sh)
或使用 wget:
bashbash <(wget -qO- https://raw.githubusercontent.com/你的用户名/scripts/main/change_ssh_port.sh)
方法2: 下载后执行
bash# 下载脚本
...
不要关闭当前连接: 修改完成后，先用新端口测试连接成功，再关闭旧连接
云服务器安全组: 如果使用阿里云/腾讯云/AWS等，需要在控制台的安全组中开放新端口
防火墙配置: 确保防火墙允许新端口的TCP连接
备份配置: 脚本会自动备份，但建议手动再备份一次
